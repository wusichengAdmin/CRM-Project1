<%@ page import="com.huashui.crm.settings.domain.DicValue" %>
<%@ page import="java.util.List" %>
<%@ page import="java.util.Map" %>
<%@ page import="com.huashui.crm.workbench.domain.Tran" %>
...
<script type="text/javascript">
	...
	function updateIcon(stage,index1) {
		...
		//如果当前阶段可能性为0，说明 前7个阶段一定是黑圈，后面两个阶段一个是红×，一个是黑×
		...
		//如果当前阶段可能性不为0	 前7个阶段有可能是 绿圈，绿色标记，黑圈 ，后面两个阶段一定是黑×
		}else {
...
<body>
	...
	<div style="position: relative; left: 40px; top: -50px;">
		...
		<%
			...
			//如果当前阶段的可能性为0 则前7个一定是黑圈，后两个一个红×，一个黑×
			if("0".equals(currentPossibility)){

				for (int i = 0; i < dvList.size(); i++) {

					//取得遍历出来的每一个阶段以及对应的可能性
					...
					if("0".equals(listPossibility)){
						...
						if(listStage.equals(currentStage)){
							//红×------------
		%>
		<span id="<%=i%>" onclick="updateStage('<%=listStage%>','<%=i%>')" class="glyphicon glyphicon-remove mystage"
		...
		<%
						...
						//如果遍历出来的阶段的可能性不为0，则是前面七个阶段
			        }else {

		%>
		<span id="<%=i%>" onclick="updateStage('<%=listStage%>','<%=i%>')" class="glyphicon glyphicon-record mystage"
			  data-toggle="popover" data-placement="bottom"
			  data-content="<%=dv.getText()%>" style="color: #000000;"></span>
		-----------
		<%
				...
				//如果当前阶段的可能性不为0，前7个有可能是绿圈，绿色标记，黑圈，后两个一定是黑×
			}else {
		...
		<%
			               //可能性不为0，是前七个
							   ...
							   if(i==index){
							   	...
							   	//绿色标记------
		%>
		<span id="<%=i%>" onclick="updateStage('<%=listStage%>','<%=i%>')" class="glyphicon glyphicon-map-marker mystage"
			  data-toggle="popover" data-placement="bottom"
		...
		<%
							   	...
							   	//绿圈-----------
		%>
		<span id="<%=i%>" onclick="updateStage('<%=listStage%>','<%=i%>')" class="glyphicon glyphicon-ok-circle mystage"
			  data-toggle="popover" data-placement="bottom"
			  data-content="<%=dv.getText()%>" style="color: #90F790;"></span>
		-----------
		<%
	...
	<div style="position: relative; top: 0px;">
		...
		<div style="position: relative; left: 40px; height: 30px; top: 70px;">
			...
			<div style="width: 500px;position: relative; left: 200px; top: -20px;"><b id="editBy">${t.editBy}&nbsp;&nbsp;</b id="editTime"><small style="font-size: 10px; color: gray;">${t.editTime}</small></div>
			<div style="height: 1px; width: 550px; background: #D5D5D5; position: relative; top: -20px;"></div>
		</div>
